...
function showPriceDetails(plan) {
  ...
  document.querySelector(
    ".order-amount"
  ).innerText = `${formattedAmount} por ${plan.interval == 'month'? 'mês': plan.interval}`;
}
...
var orderComplete = function(subscription) {
  ...
  document.querySelector(".order-status").textContent = subscription.status == 'active' ? 'ativa' : subscription.status;
